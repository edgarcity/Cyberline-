...
// Configuration des broches pour le Bluetooth
...
void ModeVirageGauche() {
  Serial.println("Virage à gauche détecté");
  enVirageGauche = true;
  MotorD(100, "avant");
  MotorG(90, "arriere");
  while (digitalRead(D1) == 1) {
    // Continue le virage tant que CD1 détecte quelque chose
  }
  enVirageGauche = false;
}

void ModeVirageDroite() {
  Serial.println("Virage à droite détecté");
  enVirageDroite = true;
  MotorD(90, "arriere");
  MotorG(100, "avant");
  while (digitalRead(D5) == 1) {
    // Continue le virage tant que CD5 détecte quelque chose
  }
  enVirageDroite = false;
}
...
void Avancer() {
  ...
  // Interception pour virages nets à gauche ou droite
  if (CD5 == 1 && CD1 == 0) {
    ModeVirageDroite();
    return;
  } else if (CD1 == 1 && CD5 == 0) {
  ...
  if (CD1 == 0 && CD2 == 0 && CD3 == 1 && CD4 == 0 && CD5 == 0) {
    MotorD(86, "avant"); // c'est pas les même car le moteur droit avance moins vite
    MotorG(80, "avant");
